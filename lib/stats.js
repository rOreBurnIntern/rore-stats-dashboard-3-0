...
function normalizeWinnerType(roundRaw) {
  const winnerTakeAllRaw = firstDefined(roundRaw, [
    "winnerTakeAll",
    "winner_take_all",
    "isWinnerTakeAll",
    "is_winner_take_all"
  ]);
  if (typeof winnerTakeAllRaw === "boolean") {
    return winnerTakeAllRaw ? "winner_take_all" : "split";
  }
  if (typeof winnerTakeAllRaw === "string") {
    const normalizedWinnerTakeAll = winnerTakeAllRaw.trim().toLowerCase();
    if (normalizedWinnerTakeAll === "true") {
      return "winner_take_all";
    }
    if (normalizedWinnerTakeAll === "false") {
      return "split";
    }
  }
...
function winnerDistribution(rounds) {
  ...
  for (const round of rounds) {
    if (round.winnerType === "split" || round.winnerTakeAll === false) {
...
function winsPerBlock(rounds) {
  const bins = Array.from({ length: BLOCK_COUNT }, (_, i) => ({ block: i, wins: 0 }));
  for (const round of rounds) {
    if (Number.isInteger(round.winnerBlock) && round.winnerBlock >= 0 && round.winnerBlock < BLOCK_COUNT) {
